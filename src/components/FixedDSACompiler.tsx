...
const FixedDSACompiler: React.FC<FixedDSACompilerProps> = ({ 
  problem, 
  onSubmit, 
  timeLimit = 45,
  companyName = 'Company';
                  ...
                  <div className="space-y-2">
                    {problem.testCases.slice(0, 3).map((testCase, index) => (
                      <div key={index} className="bg-gray-50 p-3 rounded text-sm">
                        <div className="font-medium text-gray-600 mb-1">Test Case {index + 1}:</div>
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <span className="text-xs text-gray-500">Input:</span>
                            <code className="block bg-white p-1 rounded text-xs">{testCase.input}</code>
                          </div>
                          <div>
                            <span className="text-xs text-gray-500">Expected:</span>
                            <code className="block bg-white p-1 rounded text-xs">{testCase.expectedOutput}</code>
                          </div>
                        </div>
                      </div>
                    ))}
                    {problem.testCases.length > 3 && (
                      <p className="text-xs text-gray-500 text-center">
                        +{problem.testCases.length - 3} more test cases (including hidden ones)
                      </p>
                    )}
                  </div>
                )}
