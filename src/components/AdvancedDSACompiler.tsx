...
interface AdvancedDSACompilerProps {
  problem: DSAProblem
...
const AdvancedDSACompiler: React.FC<AdvancedDSACompilerProps> = ({ 
  problem, 
  onSubmit, 
  timeLimit = 45;
}) => {
  ...
  // Enhanced language templates with better starter code
...
int main() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(`dsa_code_${problem.id}_${language}`);
  ...
  const validateSyntax = async () => {
    ...
    try {
      const validation = await judge0Service.validateSyntax(code, language);
  ...
  const runCode = async () => {
    ...
    setIsRunning(true);
    const toastId = toast.loading('Validating and executing your code...');
    ...
    try {
      ...
      const avgTime = results.reduce((sum, r) => sum + (parseFloat(r.executionTime || '0') || 0), 0) / results.length;
      ...
      if (passedCount === totalCount) {
        toast.success(`🎉 All ${totalCount} test cases passed!`, {
          description: `Avg execution time: ${avgTime.toFixed(2)}ms`
        })
      } else {
        toast.warning(`⚠️ ${passedCount}/${totalCount} test cases passed`, {
          description: 'Check failed test cases for details'
        })
      }
    } catch (error) {
      toast.dismiss(toastId);
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div className="text-center p-3 bg-purple-50 rounded">
                    <div className="text-2xl font-bold text-purple-600">
                      {codeExecutionStats.averageExecutionTime.toFixed(1)}ms
                    </div>
...
export default AdvancedDSACompiler;